...
const REGISTRY: Record<Exclude<DispatchDept, 'NONE'>, string[]> = {
...
export function getContactForDepartment(dept: DispatchDept, salt = 0): string {
  if (dept === 'NONE') return 'No external dispatch (informational)';
  const list = REGISTRY[dept];
  return pick(list, salt);
}

export function buildDispatchContacts(departments: DispatchDept[], salt = 0): DispatchContact[] {
