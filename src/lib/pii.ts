...
export function redactPii(text: string): PiiRedactionResult {
  let redacted = text;
  const flags = { names: 0, phones: 0, addresses: 0, aadhaar: 0, schools: 0 };
  ...
  return { redactedText: redacted, piiFlags: flags };
}

export function piiFlagsToJson(flags: PiiRedactionResult['piiFlags']): string {
  return JSON.stringify({ name: flags.names, phone: flags.phones, address: flags.addresses, aadhaar: flags.aadhaar, school: flags.schools });
}
