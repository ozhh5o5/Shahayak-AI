...
export async function classifyTurn(
  ...
  const draftResponses = buildDraftResponses(intent, priorState.callerTurnCount);
...
function buildDraftResponses(intent: Intent, callerTurns: number) {
  return [
...
export async function generateRestatement(case_: CaseLikeForAi): Promise<string> {
  ...
  const baseMessage = intentMap[case_.intent || 'OTHER'];
  return `${baseMessage} Is this correct?`;
}
